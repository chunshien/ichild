...
import {Animated, StyleSheet, Text, View, Modal, StatusBar, WebView, Linking,
...
export default class ImageSlider extends React.Component {
  defaultSource = "https://upload.wikimedia.org/wikipedia/commons/thumb/a/ac/No_image_available.svg/600px-No_image_available.svg.png"
  //props
  id = "imageSlider"
  data = []
  ...
  _handleDynamicStyles(){
    ...
    var titleSizeStyle = {
      fontSize: Math.floor(this.width/17),
      lineHeight: Math.floor(this.width/15),
    };
    var descriptionSizeStyle = {
      fontSize: Math.floor(this.width/25)
    };
    //slider style
    var sliderImageSize = Math.floor(this.width/17);
    var paddingVertical = Math.floor(this.width/18);
    ...
    var sliderPositionVertical = Math.floor((this.height - sliderContainerHeight)/2)
    ...
    //navigation style
    var circleSize = Math.floor(this.width/40);
  ...
  _handleInitDynamicStyleAndDefaultAnim(){
    ...
    //x,y
    this.dynamicStyles = this._handleDynamicStyles();
  ...
  render(){
    this._initStyle();
    if(this.width != 0 && this.height != 0 /*&& (this.autoResize && this.allGetSize || !this.autoResize)*/) {
      {
        //image styles
        ...
        if(this.data != undefined && this.data.length > 0){
          ...
          var renderAnimatedImage = (index, data, style) => {
            ...
            var renderLazyLoadImage = () => {
              if(data.image.indexOf("https://www.youtube.com/watch")>-1){
            ...
            return(
              <Animated.View
                key={`image-${index}`}
                style={[styles.animatedImageContainer, imageSizeStyle, style, {
                  transform: [
          ...
          var renderAnimatedImages = () => {
            return Object.keys(this.data).map((index)=>{
              return(
                <View key={index}>
                  {renderAnimatedImage(index, this.data[index], this.imageStyles[index])}
                </View>
              )
            })
          }
          ...
          if((this.data[this.showIndex].title != undefined && this.data[this.showIndex].title.length > 0) || (this.data[this.showIndex].description != undefined && this.data[this.showIndex].description.length > 0)){
            return(
              <Animated.View style={[styles.descriptionContainer, descriptionBelowStyle /*this.descriptionBelowStyle*/]}>
                {this.data[this.showIndex].title != undefined && this.data[this.showIndex].title.length > 0 ?
